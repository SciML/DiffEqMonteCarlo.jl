...
sim = solve(prob2,SRIW1(),DiffEqMonteCarlo.MonteThreads(),dt=1//2^(3),num_monte=10,parallel_type=:threads)
DiffEqBase.calculate_monte_errors(sim)
@test length(sim) == 10

sim = solve(prob2,SRIW1(),DiffEqMonteCarlo.MonteSplitThreads(),dt=1//2^(3),num_monte=10,parallel_type=:split_threads)
DiffEqBase.calculate_monte_errors(sim)
@test length(sim) == 10

sim = solve(prob2,SRIW1(),DiffEqMonteCarlo.MonteSerial(),dt=1//2^(3),num_monte=10,parallel_type=:none)
DiffEqBase.calculate_monte_errors(sim)
@test length(sim) == 10
